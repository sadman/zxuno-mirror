...
void copyfromfile2spi (BYTE handle, WORD kbytes);
...
BYTE commandlinemode (char *p)
{
    ...
    copyfromfile2spi (handle, kbytes);
...
void usage (void)
{
    ...
    puts (" WRITESPI file.bin size_in_kb\xd\xd"
          "Writes size_in_kb kbytes to a\xd"
          "SPI Flash from a file.\xd");
}
...
{
    WORD i,by;
    WORD paginas = kbytes<<2;

    for (i=0;i<paginas;i++)
    {
        if ((i&0xf)==0)   // Borramos un sector (4KB)
        {
           enablespi();
           writespi(0x06);
           disablespi();
        ...
        for (by=0;by<256;by++)
            ZXUNODATA = buffer[by];  // writespi rapidito
        disablespi();
        waitstatus();
