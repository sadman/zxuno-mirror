...
else if (pla[61] && !pla[58] && !pla[59]) begin
    case (mtstate)
    M1T1: begin 
...
else if (~use_ixiy && pla[70] && !pla[55]) begin
    case (mtstate)
    M1T1: begin 
...
else if (~use_ixiy && pla[72] && !pla[55]) begin
    case (mtstate)
    M1T1: begin 
                    ctl_flags_alu=1; /* Load FLAGT from the ALU */
                    ctl_alu_oe=1; /* Enable ALU onto the data bus */
...
else if (~use_ixiy && pla[74] && !pla[55]) begin
    case (mtstate)
    M1T1: begin 
                    ctl_reg_gp_we=1; ctl_reg_gp_sel=op21; ctl_reg_gp_hilo={!rsel0,rsel0}; /* Write 8-bit GP register selected by op[2:0] */
                    ctl_reg_in_hi=1; ctl_reg_in_lo=1; /* From the ALU side into the register file */
...
else if (~use_ixiy && pla[73] && !pla[55]) begin
    case (mtstate)
    M1T1: begin 
                    ctl_reg_gp_we=1; ctl_reg_gp_sel=op21; ctl_reg_gp_hilo={!rsel0,rsel0}; /* Write 8-bit GP register selected by op[2:0] */
                    ctl_reg_in_hi=1; ctl_reg_in_lo=1; /* From the ALU side into the register file */
